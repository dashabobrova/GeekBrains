...
const gallery = {
  ...
  settings: {
    ...
    openedImageCloseBtnSrc: 'images/gallery/close.jpg',
...
//Инициализация галереи и обработчик события
  init(settings) {
    ...
    document
      ...
      .addEventListener('click', event => this.containerClickHandler(event));//открытие полноразмерной картинки при клике
  },

//Обработчик события клика для открытия картинки при клике прям на картинку
...
//Открывает картинку.
  openImage(src) {
    ...
    // целевого тега (data-full_image_url аттрибут в html), если картинка не загрузилась - показываем картинку-заглушку.
...
//Создает контейнер для открытой картинки.
  ...
  //Возвращает предыдущий элемент (картинку) от открытой или последнюю картинку в контейнере (если текущая открытая картинка первая)
  getPrevImage() {
...
// Инициализация галереипри загрузке страницы
